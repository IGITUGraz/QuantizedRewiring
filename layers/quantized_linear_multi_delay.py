...
def random_removal_strategy_v2(module):
    C = torch.round(module.C_shadow.data)
    max_fan_in = module.rewiring_config['max_fan_in']
    fan_in = torch.sum(C, dim=(1, 2))
...
def forward_pre_hook(module, input):
    ...
    module.weight_delayed = torch.where(
        module.D,
        torch.broadcast_to(module.weight[..., None], module.D.shape),
        torch.zeros_like(module.D, dtype=module.weight.dtype)
    )
...
class QLinearMultiDelay(torch.nn.Module):
    def __init__(self, in_features: int, out_features: int, rewiring_config: Dict, bias: bool = True,
        ...
        self.max_delay = self.rewiring_config['max_delay']
        self.register_buffer('delay', torch.randint(1, self.max_delay, (out_features, in_features), **factory_kwargs,
                                                    requires_grad=False))
        self.register_buffer('delay_range', torch.arange(1, self.max_delay + 1, device=device, requires_grad=False))
        self.register_buffer('D', torch.clone(
            torch.broadcast_to(self.delay[..., None], (*self.delay.shape, self.max_delay)) == self.delay_range))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = torch.einsum('bi,oid->bod', x, self.weight_delayed)
        if self.bias is not None:
